...
public class HttpWebsocketEndpointManager extends AbstractInboundEndpointManager {
    ...
    public boolean startListener(int port, String name, InboundProcessorParams inboundParameters) {
        ...
        HttpWebSocketInboundEndpointConfiguration configuration = buildConfiguration(port, name,
                inboundParameters, false);

        return HttpWebSocketInboundEndpointHandler.startEndpoint(configurationContext, configuration);
    }
    ...
    public boolean startSSLListener(int port, String name, InboundProcessorParams inboundParameters) {
        ...
        HttpWebSocketInboundEndpointConfiguration configuration = buildConfiguration(port, name,
                inboundParameters, true);

        return HttpWebSocketInboundEndpointHandler.startSSLEndpoint(configurationContext, configuration);
    }
     ...
     */
    public static HttpWebSocketInboundEndpointConfiguration buildConfiguration(int port, String name,
                                                                               InboundProcessorParams params,
                                                                               boolean isSSL) {

        HttpWebSocketInboundEndpointConfiguration configuration = new HttpWebSocketInboundEndpointConfiguration();
        configuration.setPort(port);
        configuration.setEndpointName(name);
        configuration.setInboundEndpointHandlers(params.getHandlers());
        configuration.setWorkerPoolConfiguration(buildWorkerPoolConfiguration(params));
        if (isSSL) {
    ...
    public static SSLConfiguration buildSSLConfiguration(InboundProcessorParams params) {
        ...
        String keyStoreParam = params.getProperties().getProperty(InboundHttpWebSocketConstants.KEY_STORE);
        String trustStoreParam = params.getProperties().getProperty(InboundHttpWebSocketConstants.TRUST_STORE);
        String clientAuthParam = params.getProperties().getProperty(InboundHttpWebSocketConstants.SSL_VERIFY_CLIENT);
        String sslProtocol = params.getProperties().getProperty(InboundHttpWebSocketConstants.SSL_PROTOCOL);
        String httpsProtocols = params.getProperties().getProperty(InboundHttpWebSocketConstants.HTTPS_PROTOCOL);
        String certificateRevocation = params.getProperties().getProperty(InboundHttpWebSocketConstants.CLIENT_REVOCATION);
        String preferredCiphers = params.getProperties().getProperty(InboundHttpWebSocketConstants.PREFERRED_CIPHERS);
        ...
        return new SSLConfiguration.SSLConfigurationBuilder().keyStore(keyStoreParam).trustStore(trustStoreParam)
                ...
                .preferredCiphersEl(preferredCiphers).build();
    }

    /**
     * Builds the {@code WorkerPoolConfiguration} to wrap all the worker pool related properties
     * defined in the inbound endpoint.
     *
     * @param params parameters defined in the inbound endpoint definition
     * @return {@code WorkerPoolConfiguration}
     */
    public static WorkerPoolConfiguration buildWorkerPoolConfiguration(InboundProcessorParams params) {

        String workerPoolCoreSize =
                params.getProperties().getProperty(InboundHttpWebSocketConstants.INBOUND_WORKER_POOL_SIZE_CORE);
        String workerPoolMaxSize =
                params.getProperties().getProperty(InboundHttpWebSocketConstants.INBOUND_WORKER_POOL_SIZE_MAX);
        String workerPoolKeepAliveSeconds =
                params.getProperties().getProperty(InboundHttpWebSocketConstants.INBOUND_WORKER_THREAD_KEEP_ALIVE_SEC);
        String workerPoolQueueLength =
                params.getProperties().getProperty(InboundHttpWebSocketConstants.INBOUND_WORKER_POOL_QUEUE_LENGTH);
        String workerPoolThreadGroup =
                params.getProperties().getProperty(InboundHttpWebSocketConstants.INBOUND_THREAD_GROUP_ID);
        String workerPoolThreadId = params.getProperties().getProperty(InboundHttpWebSocketConstants.INBOUND_THREAD_ID);

        if (workerPoolCoreSize != null && workerPoolMaxSize != null && workerPoolKeepAliveSeconds != null
                && workerPoolQueueLength != null && workerPoolThreadGroup != null && workerPoolThreadId != null) {
            return new WorkerPoolConfiguration(workerPoolCoreSize, workerPoolMaxSize, workerPoolKeepAliveSeconds,
                    workerPoolQueueLength, workerPoolThreadGroup, workerPoolThreadId);
        }
        return null;
    }
}
